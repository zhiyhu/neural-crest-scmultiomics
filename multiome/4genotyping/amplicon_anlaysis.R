...
# naive bayes
#---------------------------
## fitting the svm (not working)
# library(e1071)
# nb.fit <- naiveBayes(formula(paste0("y~", paste(colnames(mat_all), collapse = "+"))), 
#                      data = training_set) 
# nb.fit
# 
# coef(nb.fit)
# 
# glm.probs <- predict.glm(nb.fit, type = "response")
# 
# pdf("genotyping/figures/boxplot_NaiveBayes_testingSet.pdf")
# testing.pred <- predict(nb.fit, newdata = data.frame(testing_mat), type = "response")
# plot(x = factor(as.character(testing_set[,1])), y = testing.pred)
# dev.off()
# 
# pdf("genotyping/figures/roc_NaiveBayes.pdf")
# pROC_obj <- roc(testing_mat[,1], testing.pred,
#                 smoothed = TRUE,
#                 # arguments for ci
#                 ci=TRUE, ci.alpha=0.9, stratified=FALSE,
...
#------------------------------------------
## manual ways to classify (not in use)
#------------------------------------------

# wt <- unique(c(rownames(txi[[1]])[txi[[1]][,3] > 0], 
#                rownames(txi[[2]])[txi[[2]][,3] > 0], 
#                rownames(txi[[3]])[txi[[2]][,3] > 0],
#                rownames(txi[[4]])[(txi[[1]][,4] + txi[[1]][,5]) == 0 & txi[[1]][,3] < 0]))
# 
# mut <- unique(c(rownames(txi[[4]])[txi[[4]][,4] + txi[[4]][,5] > 0 & txi[[4]][,3] == 0], 
#                 rownames(txi[[6]])[txi[[6]][,4] > 0],
#                 rownames(txi[[7]])[txi[[7]][,4] > 0]))
# 
# df_samples$genotype[is.na(df_samples$genotype) & 
#                       df_samples$barcode %in% wt &
#                       !df_samples$barcode %in% mut] <- "wt"
# 
# df_samples$genotype[is.na(df_samples$genotype) & 
#                       !df_samples$barcode %in% wt &
#                       df_samples$barcode %in% mut] <- "mut"
# 
# df_samples$genotype[is.na(df_samples$genotype) & 
#                       df_samples$barcode %in% wt &
#                       df_samples$barcode %in% mut] <- "unknown"
# 
# df_samples$cell_id <- paste0("s8_",df_samples$barcode, "-1_",df_samples$barcode, "-1")
# metadata_s8 <- metadata[metadata$sample == "4mix",]
# sum(df_samples$cell_id %in% rownames(metadata_s8))
# 
# table(df_samples$genotype[df_samples$cell_id %in% rownames(metadata_s8)])
# # mut unknown      wt 
# # 101      26     216
# 
# df_samples2 <- df_samples[df_samples$cell_id %in% rownames(metadata_s8),]
# 
# metadata$predicted_genotype <- NA
# metadata$predicted_genotype[match(df_samples2$cell_id, rownames(metadata))] <- df_samples2$genotype
