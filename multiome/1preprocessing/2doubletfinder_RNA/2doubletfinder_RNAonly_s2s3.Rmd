...
author: "Zhiyuan Hu"
date: '2022-12-21 (last modified: `r Sys.Date()`)'
...
```{r}
projname <- "12-16ss"
s2 <- readRDS("/t1-data/project/tsslab/zhu/multiome/analysis_newref/preprocessing/rds/intermediate_seuobj/post_initialqc_scmo_s2_RNA.rds")
s3 <- readRDS("/t1-data/project/tsslab/zhu/multiome/analysis_newref/preprocessing/rds/intermediate_seuobj/post_initialqc_scmo_s3_RNA.rds")
seu <- merge(s2, y = s3, project = projname)
...
seu <- doubletFinder_v3(seu, PCs = 1:30, pN = 0.25, pK = 0.03, nExp = nExp_poi, reuse.pANN = FALSE, sct = FALSE)
seu <- doubletFinder_v3(seu, PCs = 1:30, pN = 0.25, pK = 0.03, nExp = nExp_poi.adj, reuse.pANN = FALSE, sct = FALSE)
...
idx <- grep("DF.classifications_0.25_0.03", colnames(seu@meta.data))
seu$DoubletFinder_pK003 <- "Singlet"
seu$DoubletFinder_pK003[seu@meta.data[,idx[1]] == "Doublet" & seu@meta.data[,idx[2]] == "Doublet"] <- "Doublet (high likelyhood)"
seu$DoubletFinder_pK003[(seu@meta.data[,idx[1]] != "Doublet" & seu@meta.data[,idx[2]]== "Doublet" )|
                    (seu@meta.data[,idx[2]] != "Doublet" & seu@meta.data[,idx[1]]== "Doublet" )] <- "Doublet (low likelyhood)"
...
p2 <- DimPlot(seu, group.by = "DoubletFinder_pK003")
...
p2 <- VlnPlot(seu, features = c("nFeature_RNA"), group.by = "DoubletFinder_pK003" , pt.size =  0)
...
```{r}
saveRDS(seu, file = "/t1-data/project/tsslab/zhu/multiome/analysis_newref/preprocessing/rds/doubletfinder/seu_rna_soupx_12_16ss.rds")
saveRDS(seu@meta.data, file = "/t1-data/project/tsslab/zhu/multiome/analysis_newref/preprocessing/rds/doubletfinder/metadata_soupx_12_16ss.rds")
```
