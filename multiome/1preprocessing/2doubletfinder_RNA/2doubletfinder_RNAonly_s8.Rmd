...
```{r}
projname <- "epiboly-4ss"
seu <- readRDS("/t1-data/project/tsslab/zhu/multiome/analysis_newref/preprocessing/rds/intermediate_seuobj/post_initialqc_scmo_s8_RNA.rds")
head(colnames(seu))
...
p2 <- DimPlot(seu, group.by = "DoubletFinder_pK")
p1 + p2
```

```{r, fig.width=12, fig.height=4}
p1 <- VlnPlot(seu, features = c("nFeature_RNA"), group.by = "DoubletFinder" , pt.size =  0)
p2 <- VlnPlot(seu, features = c("nFeature_RNA"), group.by = "DoubletFinder_pK" , pt.size =  0)
cowplot::plot_grid(p1, p2, ncol = 2)
```

```{r}
table(seu$DoubletFinder)
```


```{r}
saveRDS(seu, file = "/t1-data/project/tsslab/zhu/multiome/analysis_newref/preprocessing/rds/doubletfinder/seu_rna_epiboly_4ss.rds")
saveRDS(seu@meta.data, file = "/t1-data/project/tsslab/zhu/multiome/analysis_newref/preprocessing/rds/doubletfinder/metadata_soupx_epiboly_4ss.rds")
```
