...
```{r}
projname <- "18-22ss"
s1 <- readRDS("/t1-data/project/tsslab/zhu/multiome/analysis_newref/preprocessing/rds/intermediate_seuobj/post_initialqc_scmo_s1_RNA.rds")
head(colnames(s1))
s6 <- readRDS("/t1-data/project/tsslab/zhu/multiome/analysis_newref/preprocessing/rds/intermediate_seuobj/post_initialqc_scmo_s6_RNA.rds")
head(colnames(s6))
s7 <- readRDS("/t1-data/project/tsslab/zhu/multiome/analysis_newref/preprocessing/rds/intermediate_seuobj/post_initialqc_scmo_s7_RNA.rds")
head(colnames(s7))
seu <- merge(s1, y = c(s6, s7), project = projname)
...
seu <- doubletFinder_v3(seu, PCs = 1:30, pN = 0.25, pK = 0.08, nExp = nExp_poi, reuse.pANN = FALSE, sct = FALSE)
seu <- doubletFinder_v3(seu, PCs = 1:30, pN = 0.25, pK = 0.08, nExp = nExp_poi.adj, reuse.pANN = FALSE, sct = FALSE)
...
idx <- grep("DF.classifications_0.25_0.08", colnames(seu@meta.data))
seu$DoubletFinder_pK008 <- "Singlet"
seu$DoubletFinder_pK008[seu@meta.data[,idx[1]] == "Doublet" & seu@meta.data[,idx[2]] == "Doublet"] <- "Doublet (high likelyhood)"
seu$DoubletFinder_pK008[(seu@meta.data[,idx[1]] != "Doublet" & seu@meta.data[,idx[2]]== "Doublet" )|
                    (seu@meta.data[,idx[2]] != "Doublet" & seu@meta.data[,idx[1]]== "Doublet" )] <- "Doublet (low likelyhood)"
...
p2 <- DimPlot(seu, group.by = "DoubletFinder_pK008")
...
p2 <- VlnPlot(seu, features = c("nFeature_RNA"), group.by = "DoubletFinder_pK008" , pt.size =  0)
...
```{r}
saveRDS(seu, file = "/t1-data/project/tsslab/zhu/multiome/analysis_newref/preprocessing/rds/doubletfinder/seu_rna_soupx_18_22ss.rds")
saveRDS(seu@meta.data, file = "/t1-data/project/tsslab/zhu/multiome/analysis_newref/preprocessing/rds/doubletfinder/metadata_soupx_18_22ss.rds")
```
