...
setwd("/home/huzhiy/projects_ox/multiome/analysis_newref/latent_time_grn")
work_dir = '/home/huzhiy/projects_ox/multiome/analysis_newref/GRN_scenicplus/ncall_2023oct_ccb/output/'
...
regulon_sets <- read.csv("/home/huzhiy/projects_ox/multiome/analysis_newref/regulon_analysis/figures/cluster_regulons/clustering_results_rb.csv")
...
for(itor_set in unique(regulon_sets$Agglomerative)){
 ...
 set_n 
  ...
  # save the plot
  pdf(paste0("/home/huzhiy/projects_ox/multiome/analysis_newref/latent_time_grn/figures/process_regulon_data/scatterplots_rb_",itor_set,".pdf"), onefile = TRUE, width = 5, height = 3)
...
for(i in 1:length(regulon_sets$Regulon)){
  ...
  ggsave(plot = p, filename = paste0("/home/huzhiy/projects_ox/multiome/analysis_newref/latent_time_grn/figures/regulon_auc_rb_scatterplots/",itor_regulon,".png"), width = 6, height = 6)
}

# save the data into csv files
latent_time_df <- data.frame(cell_id = idx,
                             latent_time = mv_time$latent_time[match(idx, mv_time$X)],
                             cell_type = mv_time$cell_type[match(idx, mv_time$X)])
write.csv(x = latent_time_df, "/home/huzhiy/projects_ox/multiome/analysis_newref/latent_time_grn/figures/process_regulon_datamv_time_clean.csv", row.names = FALSE)

auc_df <- auc[match(idx, auc_cells), auc_regulons %in% regulon_sets$Regulon]
write.csv(x = auc_df, "/home/huzhiy/projects_ox/multiome/analysis_newref/latent_time_grn/figures/process_regulon_data/auc_rb_clean.csv", row.names = FALSE)
