# Run Sparpros on Multiome NC data
...
adata = sc.read("velocity/data/seurat/seu_RNAsoupx_NC.h5ad")
adata.var_names = adata.var['_index']
adata.var_names

sc.pl.violin(adata, ['tfec', 'fli1a'], groupby='cell_type', use_raw = False )
...
import os
os.chdir("STprobe_select/figures/multiome/")
sc.pl.violin(adata, ['tfec', 'fli1a'], groupby='cell_type', use_raw = False, save="violinPlots_tfec_fli1a.png")
...
selector = sp.se.ProbesetSelector(
    ...
    n_jobs=-1,
    verbosity=1,
    save_dir="STprobe_select/results/multiome/"
)
...
# with open('STprobe_select/results/multiome/selector.pkl', 'wb') as outp:
