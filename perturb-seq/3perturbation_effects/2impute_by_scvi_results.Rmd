---
title: "Impute latent time by scVI"
...
```{r}
df_scvi <- read.csv("perturb_seq/analysis/integration/data/integrated/multiomeNCwt_perturbseq_scvi_data.csv", row.names = 1)
head(df_scvi)
```
...
```{r}
table(colnames(seu_merged) == rownames(df_scvi))
```


```{r}
seu_merged@reductions[["scvi_pc"]] <- CreateDimReducObject(embeddings = as.matrix(df_scvi), key = "scvi_pc")
```

```{r}
seu_merged <- RunTSNE(seu_merged, reduction = "scvi_pc", dims = 1:10)
```
...
seu_merged <- FindNeighbors(seu_merged,reduction = "scvi_pc", dims = 1:10)  # You can adjust the dims
```
...
```{r}
saveRDS(seu_merged, "../data/integrated/scvi_perturbseq_multiome_latentTime.rds")
```
